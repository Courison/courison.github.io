...
(function() {
    ...
    // ---------- 常量 ----------
    const PET_IMG_URL    = '/assets/images/pet.webp';
    const PET_SQUISH_URL = '/assets/images/pet-squish.webp';
    const USE_IMAGES = true;

    const POSITION_KEY = 'dijiang_pet_position';
    const SIZE_KEY     = 'dijiang_pet_size';
    ...
    // 允许动态变化的图片 URL（webp 加载失败时切 png）
    let currentPetImg    = PET_IMG_URL;
    let currentPetSquish = PET_SQUISH_URL;
    ...
    function createWidget() {
        ...
        let spriteInner = '';
        if (USE_IMAGES) {
            spriteInner = `<img class="pet-img" src="${currentPetImg}" alt="捏捏" draggable="false">`;
        } else {
    ...
    function setupImageFallback() {
        ...
        petImg.addEventListener('error', function onErr() {
            if (!triedPng && this.src && /\.webp(\?.*)?$/i.test(this.src)) {
                ...
                if (/\.webp(\?.*)?$/i.test(currentPetImg)) currentPetImg = currentPetImg.replace(/\.webp(\?.*)?$/i, '.png');
                if (/\.webp(\?.*)?$/i.test(currentPetSquish)) currentPetSquish = currentPetSquish.replace(/\.webp(\?.*)?$/i, '.png');
                this.src = pngUrl;
                return;
            }
    ...
    function playSquishAnimation() {
        ...
        if (petImg && petImg.tagName === 'IMG' && USE_IMAGES && currentPetSquish && currentPetSquish !== currentPetImg) {
            petImg.src = currentPetSquish;
        }
